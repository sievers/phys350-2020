...
def deres_mat(mat,dosum=False):
    ...
    if dosum:
        mm=mat[::2,::2]+mat[1::2,::2]+mat[::2,1::2]+mat[1::2,1::2]
    else:
...
def solve_system(bc,mask,rho=None,npass=6,niter=150,return_full=False,plot_steps=False):
...
if False:
    ...
    bc[npix//2,-npix//3:]=1
    mask[npix//2,-npix//3:]=True
    pot=solve_system(bc,mask)
if False:
    rho=0*bc
    rho[npix//2,npix//16]=1
    pot=solve_system(bc,mask,rho)
